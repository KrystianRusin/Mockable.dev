...
const generatePOSTData = async (
  ...
  const cacheKey = `post:${cacheHash}`;

  if (!forceRefresh) {
    try {
      const cachedData = await redisClient.get(cacheKey);
      if (cachedData) {
        console.log('Returning cached POST response data');
        ...
        messages: [
          {
            role: "system",
            content: `
              You are a data generator that strictly adheres to the provided JSON schemas. When generating response data:
